...
/* 
 * Global variables
 * Including kv-store, queue and five semaphores
 */
/* kv-store */
Skiplist* Items;
/* queue */
...
void data_work(int fd)
{
	...
	while (run) {
		...
		if (cmd == D_PUT) {
			...
			err = exists_skiplist(Items, key);
			if (err == 0) {

				/* Insert command. */
				err = insert_skiplist(Items, key, text);
				if (err == 0) {
				...
				/* Update command. */
				err = update_skiplist(Items, key, text);
				if (err == 0) {
					strncpy(buffer, "Stored key successfully.\n", LINE);
				} else {
					strncpy(buffer, "Error storing key.\n", LINE);
			...
			/* GET command. */
			err = search_skiplist(Items, key, copy);
			if (err == 0) {
				strcat(copy, "\n");
				...
				/* Error for search key. */
				strncpy(buffer, "Error in search function.\n", LINE);
			}

		} else if (cmd == D_COUNT) {

			/* Count command. */
			sprintf(copy, "%d\n", count_skiplist(Items));
			strncpy(buffer, copy, LINE);

		} else if (cmd == D_DELETE) {

			/* Delete command. */
			err = delete_skiplist(Items, key);
			if (err == 0) {
