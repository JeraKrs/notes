...
int main(int argc, char* argv[])
{
	...
	distribute(params.ny, nprocs, rank, &params.offset, &params.size);
	params.size = params.size - params.offset;

	int start = params.offset, stop = params.offset + params.size;
	int l_nb = (rank - 1 + nprocs) % nprocs;
	int l_bf = (start - 1 + params.ny) % params.ny;
	int r_nb = (rank + 1) % nprocs;
	int r_bf = stop % params.ny;

	size_t numWorkGroups = params.size*params.nx / (LOCAL_SIZE[0]*LOCAL_SIZE[1]);
	...
	size_t global_move[2] = {params.size, params.nx};
	for (params.tt = 0; params.tt < params.maxIters; params.tt++)
	{
		/* Flow */
		...
		clEnqueueReadBuffer(ocl.queue, ocl.d_cells, CL_TRUE,
				sizeof(cl_float)*NSPEEDS*params.nx*offset,
				sizeof(cl_float)*params.nx*NSPEEDS,
				cells+offset*params.nx, 0, NULL, NULL);
		offset = params.offset + params.size - 1;
		clEnqueueReadBuffer(ocl.queue, ocl.d_cells, CL_TRUE,
				sizeof(cl_float)*NSPEEDS*params.nx*offset,
				sizeof(cl_float)*params.nx*NSPEEDS,
				cells+offset*params.nx, 0, NULL, NULL);
	...
	for (int tt = 0; tt < params.maxIters; tt++) {
		...
		MPI_Reduce(&tot_u, &av_vels[tt], 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
		av_vels[tt] /= (float)total_cell;
	}
	...
	if (rank != MASTER) {
		MPI_Send(cells+params.offset*params.nx,
				params.nx*NSPEEDS*params.size,
