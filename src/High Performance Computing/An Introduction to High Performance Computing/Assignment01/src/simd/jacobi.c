...
int run(const double *A, const double *b, double *x, double *xtmp) {
	int itr;
	double diff;
	double sqdiff;
	double *ptrtmp;
	...
	do {
		...
		for (int row = 0, ridx = 0; row < N; row++, ridx += N) {
			...
			for (int col = 0; col < N; col += 2) {
				if (col + 2 <= N) {
					vector1 = _mm_load_pd(A+ridx+col);
					vector2 = _mm_load_pd(x+col);
					_mm_store_pd(tmp, _mm_mul_pd(vector1, vector2));
					dot += tmp[0] + tmp[1];
				} else {
