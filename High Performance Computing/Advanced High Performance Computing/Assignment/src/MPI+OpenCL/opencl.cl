				   ...
				   __global			float	* ret)
{
	...
	int jj = get_global_id(0) + params.offset;
	...
	tmp[0] = cells[ii + jj*params.nx][0];
	tmp[1] = cells[x_w + jj*params.nx][1];
	tmp[2] = cells[ii + y_s*params.nx][2];
	tmp[3] = cells[x_e + jj*params.nx][3];
	tmp[4] = cells[ii + y_n*params.nx][4];
	tmp[5] = cells[x_w + y_s*params.nx][5];
	tmp[6] = cells[x_e + y_s*params.nx][6];
	tmp[7] = cells[x_e + y_n*params.nx][7];
	tmp[8] = cells[x_w + y_n*params.nx][8];

	if (obstacles[col])
	{
		tmp_cells[col][1] = tmp[3];
		tmp_cells[col][2] = tmp[4];
		tmp_cells[col][3] = tmp[1];
		tmp_cells[col][4] = tmp[2];
		tmp_cells[col][5] = tmp[7];
		tmp_cells[col][6] = tmp[8];
		tmp_cells[col][7] = tmp[5];
		tmp_cells[col][8] = tmp[6];
	}
	else
	{
